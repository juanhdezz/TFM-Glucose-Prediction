...
#
# Grupos de edad: <31, 31-45, 46-65, >=65
#
...
# Adaptación para servidor: los datos están en data/input/
DATA_DIR = ROOT_DIR / "data" / "input"   # <--- CAMBIO RESPECTO A LOCAL
...
def process_dataset(
    ...
    # <--- CAMBIO: usar .upper() para que coincida con las claves del diccionario
    sensor_limits = DEFAULT_SENSOR_LIMITS.get(assets.dataset_name.upper(), (39.0, 401.0))

    for horizon in horizons:
        # <--- CAMBIO: búsqueda insensible a mayúsculas/minúsculas
        all_windows = list(assets.dataset_dir.glob("windows_with_5folds_*.parquet"))
        windows_candidates = []
        for f in all_windows:
            stem = f.stem
            if stem.startswith("windows_with_5folds_"):
                name_part = stem[len("windows_with_5folds_"):]
                # Verificar si el nombre del dataset (case-insensitive) está en name_part
                if assets.dataset_name.lower() in name_part.lower():
                    # Filtrar por horizonte (si se especifica)
                    if horizon is None or f"PH{horizon}" in stem:
                        windows_candidates.append(f)
        # Ordenar para tener consistencia
        windows_candidates = sorted(windows_candidates)
