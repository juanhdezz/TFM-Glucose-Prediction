...
sección~\ref{sec:analisis_mejora}.
...
El heatmap de la figura~\ref{fig:heatmap_age_diatrend} (DiaTrend) cuantifica
...
En T1DiabetesGranada el desequilibrio de partida es moderado y las variaciones 
$\Delta\%$ son de magnitud contenida, por lo que se omite su heatmap: visualmente el mapa queda casi en blanco en comparación con el de DiaTrend, sin aportar información adicional a la que se describe en el texto.
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth]{%
        ../../analysis_relief/outputs_clinical/heatmap_balancing_DIATREND_age.pdf}
    \caption{Variación $\Delta n$ y $\Delta\%$ de ventanas por técnica de
             balanceo y grupo de edad en \textbf{DiaTrend}. La saturación
             visiblemente superior refleja el desequilibrio demográfico extremo
             documentado en la sección~\ref{subsec:eda_demografico}.}
...
El heatmap de REPLACE-BG se omite por la misma razón que el de T1DiabetesGranada: con seguimientos homogéneos de $\approx$182 días, las variaciones 
$\Delta\%$ son de magnitud similar o inferior a las de T1DiabetesGranada, y el mapa resultante ofrece escaso contraste visual respecto al caso mostrado en la figura~\ref{fig:heatmap_age_diatrend}.
...
La distribución absoluta de ventanas por rango glucémico en T1DiabetesGranada se omite: dado que las variaciones relativas son en todos los casos inferiores al 
$1\%$ (como muestra la figura~\ref{fig:delta_glycemic_t1d}), las barras apiladas son visualmente indistinguibles entre técnicas y no añaden información interpretable sobre las barras divergentes.
...
La variación de la composición glucémica en T1DiabetesGranada es inferior al 
$1\%$ en todos los rangos y técnicas. El scatter de trade-off de la figura~\ref{fig:tradeoff_tbr2} muestra visualmente que los puntos de este dataset quedan apiñados en torno al origen, confirmando que el balanceo no produce reorientación glucémica clínicamente relevante en este repositorio.
...
La figura de barras apiladas de REPLACE-BG se omite por los mismos motivos que la de T1DiabetesGranada: la homogeneidad de los seguimientos produce distribuciones absolutas prácticamente idénticas entre técnicas, cuya variación relativa se recoge íntegramente en la figura~\ref{fig:delta_glycemic_replacbg}.
...
Análogamente, REPLACE-BG presenta cambios 
$\Delta\%$ inferiores a 
$\pm 0,41\%$ en todos los rangos, siendo el repositorio con menor impacto glucémico del balanceo de los tres evaluados. El scatter de la figura~\ref{fig:tradeoff_tbr2} confirma esta compresión visual.
...
esos eventos. La sección~\ref{sec:analisis_mejora} analiza si
...
los tres repositorios, pero con algunas diferencias relevantes en la
magnitud y dirección de los desplazamientos, como muestra la
figura~\ref{fig:tradeoff_tbr1}.
...
En \textbf{T1DiabetesGranada} la dispersión aumenta ligeramente respecto a
TBR-2: PA-Undersampling (edad) se desplaza hacia el cuadrante ideal con mayor
claridad ($\Delta\%\,\text{TIR} \approx +1{,}15\%$, $\Delta\%\,\text{TBR-1}$
negativo o nulo), mientras que SMOTE y RUS+SMOTE (edad) se sitúan en el
cuadrante inferior izquierdo (reducen TBR-1 y apenas modifican TIR). Jittering
mantiene su posición en el cuadrante superior derecho pero con un incremento
de TBR-1 más pronunciado que el de TBR-2, lo que indica que su efecto de
aumento es proporcionalmente mayor en hipoglucemia leve.

En \textbf{DiaTrend} la lectura cambia respecto a TBR-2: ROS, SMOTE y
Jittering (dimensión edad) se desplazan hacia el cuadrante superior derecho
con $\Delta\%\,\text{TBR-1} > 0$ y $\Delta\%\,\text{TIR} > 0$, lo que
sugiere que al amplificar el grupo $\geq$66 se introducen más ventanas en
hipoglucemia leve de lo que sucedía con la severa. PA-Undersampling mantiene
su posición favorable ($\Delta\%\,\text{TIR}$ alto, $\Delta\%\,\text{TBR-1}$
próximo a cero o negativo). Las técnicas de reducción siguen en el cuadrante
inferior izquierdo.

En \textbf{REPLACE-BG} el patrón es análogo al de TBR-2 con escala similar,
sin diferencias cualitativas notables entre rangos de hipoglucemia. En
conjunto, TBR-1 y TBR-2 muestran el mismo ranking de técnicas por proximidad
al cuadrante ideal: PA-Undersampling lidera en DiaTrend, mientras que en
T1DiabetesGranada y REPLACE-BG ninguna técnica consigue un posicionamiento
nítidamente favorable.
...
reducen la hiperglucemia leve al tiempo que aumentan el TIR.
...
En \textbf{T1DiabetesGranada} la mayoría de técnicas de aumento (ROS, SMOTE,
...
más pronunciado de todos los rangos en T1DiabetesGranada, como muestra la
figura~\ref{fig:tradeoff_tar2}.
...
En \textbf{T1DiabetesGranada} PA-Undersampling (edad) se posiciona en el
extremo superior derecho: consigue la mayor ganancia de TIR ($\approx
+1{,}15\%$) pero a costa de un incremento de TAR-2 de $\approx +0{,}49\%$,
el mayor aumento de hiperglucemia severa observado en cualquier técnica y
dataset. Este comportamiento es coherente con la lógica de PA-Undersampling
en T1DiabetesGranada: al eliminar desproporcionadamente ventanas del grupo
46--65 (que tiene un perfil glucémico con menor TAR-2), el train resultante
queda con mayor representación de perfiles de pacientes mayores que presentan,
paradójicamente, más episodios de hiperglucemia severa. Jittering (edad)
también se sitúa en el cuadrante superior derecho ($+0{,}23\%$ TAR-2,
$+0{,}10\%$ TIR), mientras que SMOTE y ROS+SMOTE quedan en el cuadrante
superior izquierdo (ideal): reducen TAR-2 entre $-0{,}14\%$ y $-0{,}23\%$
con ganancias de TIR de hasta $+0{,}46\%$.

En \textbf{DiaTrend} el patrón se invierte respecto a T1DiabetesGranada: la
mayoría de técnicas de aumento se posicionan en el cuadrante superior
izquierdo (reducen TAR-2 y aumentan TIR), con PA-Undersampling de nuevo en
el extremo pero ahora en el cuadrante ideal ($\Delta\%\,\text{TAR-2} \approx
-0{,}18\%$, $\Delta\%\,\text{TIR} \approx +1{,}15\%$). RUS cae en el
cuadrante inferior izquierdo.

En \textbf{REPLACE-BG} los puntos de dimensión edad se distribuyen entre los
cuadrantes superior izquierdo (SMOTE: $-0{,}11\%$ TAR-2) y superior derecho
(PA-Undersampling: $+0{,}20\%$ TAR-2), con amplitudes similares a las de
TAR-1.
...
y la dirección de esas diferencias es en sí misma un hallazgo metodológico.

\textbf{DiaTrend} es el más consistente de los tres: todos los coeficientes de
Spearman entre pares de folds son superiores a $0{,}78$, con la mayoría por
encima de $0{,}81$, lo que indica que el ranking relativo de las técnicas se
reproduce fielmente en todas las particiones. Este resultado puede parecer
...
heterogeneidad entre folds afecta al nivel absoluto del error (todos los métodos
suben o bajan juntos según la dificultad de la partición), pero no altera el orden
relativo de las técnicas. La causa profunda reside en la naturaleza del desequilibrio
demográfico de DiaTrend: con un ratio de 89{,}4:1 a nivel de ventana entre el
subgrupo de edad mayoritario y el minoritario, dicho desequilibrio actúa como la
señal dominante de varianza del \textit{dataset}. Cuando un sesgo es tan extremo y
sistemático, gobierna el comportamiento del optimizador de forma prácticamente
idéntica en todas las particiones, con independencia de qué pacientes concretos
compongan cada fold. El ranking de técnicas resulta así estable no a pesar del
pequeño tamaño muestral de DiaTrend (51 pacientes), sino precisamente porque la
magnitud del desequilibrio aplasta cualquier otra fuente de variabilidad
inter-fold.

\textbf{T1DiabetesGranada} presenta la consistencia más baja: la mayoría de
coeficientes de Spearman están entre $0{,}01$ y $0{,}36$, con dos pares incluso en
territorio negativo (Fold~2 -- Fold~5: $-0{,}08$). A primera vista, que el
repositorio más grande produzca los rankings menos reproducibles parece paradójico.
Sin embargo, esta inversión de la intuición tiene una explicación directa en términos
de teoría estadística del aprendizaje: en T1DiabetesGranada la heterogeneidad
fenotípica y clínica de los 643 pacientes introduce una variabilidad inter-paciente
de tal magnitud que supera en todo momento la señal del desequilibrio demográfico.
Cada fold asigna aleatoriamente al subconjunto de test un grupo diferente de
pacientes con trayectorias de glucosa, historiales de seguimiento y perfiles
fisiológicos distintos; en consecuencia, la dificultad relativa del test oscila
dinámicamente entre iteraciones, y con ella el orden de mérito de las técnicas.
La baja consistencia de T1DiabetesGranada no debe interpretarse, por tanto, como
una falta de robustez del \textit{pipeline}, sino como la huella estadística de la
riqueza representacional de una cohorte masiva de vida real: precisamente el mismo
mecanismo que explica la insensibilidad de este repositorio al balanceo demográfico
(véase Sección~\ref{subsec:discusion_granada}).

Esta pronunciada divergencia entre DiaTrend y T1DiabetesGranada ilustra una
paradoja de alcance general: \textbf{un sesgo demográfico extremo produce rankings
más estables que una cohorte demográficamente rica}, porque en el primer caso el
sesgo es la señal dominante y en el segundo es ruido de fondo frente a la
heterogeneidad clínica. El corolario práctico es inmediato: la consistencia
inter-fold de Spearman no es una medida de la calidad intrínseca del
\textit{pipeline}, sino del régimen de varianza que domina el \textit{dataset}.
Interpretar valores bajos de Spearman como síntoma de inestabilidad metodológica
sería un error; en cohortes clínicas heterogéneas, la inconsistencia del ranking
es el resultado esperado de una partición honesta.

\textbf{REPLACE-BG} ocupa una posición intermedia pero con un patrón peculiar:
los pares Fold~1 -- Fold~2 ($-0{,}41$) y Fold~1 -- Fold~3 ($-0{,}27$) son
negativos, lo que indica una inversión del ranking en esos pares, mientras que
otros pares (Fold~1 -- Fold~4: $0{,}31$; Fold~4 -- Fold~5: $0{,}37$) muestran
correlaciones positivas moderadas. Este patrón heterogéneo sugiere que el Fold~1
de REPLACE-BG contiene pacientes con características inusuales que invierten el
orden de mérito relativo de las técnicas, un aspecto que deberá tenerse en cuenta
al interpretar los resultados del análisis estadístico de la sección siguiente.

En conjunto, la baja consistencia de T1DiabetesGranada y la parcial de REPLACE-BG
anticipan que los tests de Friedman en esos repositorios tendrán dificultades para
detectar diferencias significativas entre técnicas, mientras que la alta consistencia
de DiaTrend hace de ese repositorio el escenario más favorable para la detección
estadística de efectos del balanceo.
...
El heatmap de la figura~\ref{fig:heatmap_delta_rmse_entire} muestra el
patrón global de mejora y deterioro para cada técnica y repositorio.
...
El heatmap revela un patrón fuertemente asimétrico entre repositorios.
Aunque la ganancia absoluta máxima obtenida por la técnica Age·Jittering en T1DiabetesGranada es cuantitativamente modesta ($-0.8\%$, equivalente a $\approx 0.30$ mg/dL de reducción en el RMSE global), este comportamiento debe interpretarse desde una perspectiva de seguridad computacional y clínica de primer orden. Estos resultados evidencian la \textbf{invarianza representacional de la red LSTM} implementada. El hecho de que el rendimiento global y por rangos permanezca inalterado tras inyectar perturbaciones gaussianas masivas (Jittering) o generar millones de instancias interpoladas artificialmente (SMOTE) demuestra que el espacio de características temporales aprendido por el modelo es extraordinariamente robusto. Para un despliegue en entornos de páncreas artificial en tiempo real, esta inmunidad frente a la manipulación del set de entrenamiento es una propiedad altamente deseable, ya que garantiza que la optimización demográfica no introduce comportamientos inestables ni degrada la precisión basal del sistema.

Las columnas de T1DiabetesGranada y REPLACE-BG presentan celdas de saturación casi nula (
$|\Delta| < 1\%$), lo que es la expresión visual directa de la no significación del test de Friedman en esos repositorios.
...
RMSE global depende críticamente de la dimensión demográfica utilizada.

En \textbf{REPLACE-BG} los cambios son los más pequeños de los tres
repositorios: todos los valores caen en el rango $[-0{,}3\%, +0{,}5\%]$,
sin ninguna técnica que destaque de forma consistente. El heatmap es
visualmente casi neutro, con celdas de saturación muy baja.
...
y $-0{,}2\%$ respectivamente. Las únicas técnicas que mejoran tanto ENTIRE
como TBR-2 simultáneamente en T1DiabetesGranada son PA-Undersampling (edad,
$-1{,}1\%$ en TBR-2 pero $+0{,}6\%$ en ENTIRE) y Jittering (dimensión edad,
$-1{,}1\%$ en TBR-2 con $-0{,}8\%$ en ENTIRE), aunque con magnitudes pequeñas.
...
global mínimo ($+0{,}1\%$), convirtiéndose en la única técnica de este
repositorio con un perfil clínico favorable.

\paragraph{Ranking por rango glucémico.}
La figura~\ref{fig:heatmap_ranking_rmse} completa el análisis anterior
mostrando la posición relativa de cada técnica en el ranking de RMSE para
cada combinación \textit{dataset} $\times$ rango glucémico.
...
a las de \textit{same\_size}, y estas a las de \textit{undersampling}. Como
muestran las figuras~\ref{fig:dist_rmse_oversampling}
a~\ref{fig:dist_rmse_undersampling}, esta hipótesis se contradice en dos de
...
\begin{figure}[htbp]
    ...
    \caption{Distribución del RMSE (\textit{ENTIRE}) para la familia
             \textit{oversampling} (Jittering, ROS, SMOTE, ROS+Tomek).
             Las técnicas de esta familia incrementan el volumen del
             conjunto de entrenamiento respecto a la partición original.}
...
\begin{figure}[htbp]
    ...
    \caption{Distribución del RMSE (\textit{ENTIRE}) para la familia
             \textit{same size} (RUS+ROS, RUS+SMOTE, SMOTE+Tomek).
             Estas técnicas híbridas mantienen el volumen de entrenamiento
             constante, actuando como grupo de control natural.}
...
\begin{figure}[htbp]
    ...
    \caption{Distribución del RMSE (\textit{ENTIRE}) para la familia
             \textit{undersampling} (RUS, PA-Undersampling, Tomek Links).
             Las técnicas de esta familia reducen el volumen del conjunto
             de entrenamiento eliminando ventanas del grupo mayoritario.}
...
la más informativa. La figura~\ref{fig:dist_rmse_oversampling} muestra
violines centrados entre $\approx$44 y 48\,mg/dL, con las técnicas de
dimensión edad claramente por debajo del \textit{baseline}
($45{,}05$\,mg/dL). La figura~\ref{fig:dist_rmse_samesize} presenta
violines más anchos y medianas que oscilan entre $\approx$44 y 48\,mg/dL
según la técnica, con Age·SMOTE+Tomek ($45{,}81$\,mg/dL) como la más
favorable. La figura~\ref{fig:dist_rmse_undersampling} es la más dispersa:
Age·Tomek Links obtiene la mediana más baja de todas las condiciones en
DiaTrend ($44{,}83$\,mg/dL, mejor que cualquier técnica de
\textit{oversampling}), mientras que Age·PAUndersampling y
Age·Undersampling producen los peores resultados ($48{,}22$ y
$47{,}76$\,mg/dL). Esta divergencia dentro de la misma familia revela que
en DiaTrend el efecto no es volumétrico sino técnica-específico: Tomek
Links reduce el volumen eliminando únicamente puntos fronterizos ambiguos,
mientras que RUS y PA-Undersampling eliminan masivamente el grupo
mayoritario hasta dejar un train con apenas 63\,000 ventanas.
...
El análisis del perfil de RMSE por rango glucémico
(figuras~\ref{fig:profile_oversampling}--\ref{fig:profile_undersampling})
completa la lectura anterior con una dimensión clínica que el RMSE global
no captura: si el efecto de cada familia es uniforme entre rangos o si hay
rangos que se benefician o perjudican selectivamente.
...
\begin{figure}[htbp]
    ...
    \caption{Perfil de RMSE por rango glucémico para la familia
             \textit{oversampling}. Cada celda muestra el RMSE medio sobre
             los \textit{folds} para una combinación técnica $\times$ rango
             $\times$ \textit{dataset}.}
    \label{fig:profile_oversampling}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Perfil de RMSE por rango glucémico para la familia
             \textit{same size}. El perfil intermedio de estas técnicas
             híbridas permite cuantificar en qué medida la fase de
             submuestreo cancela los beneficios de la fase de
             sobremuestreo.}
    \label{fig:profile_samesize}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Perfil de RMSE por rango glucémico para la familia
             \textit{undersampling}. Nótese la degradación en TIR y TAR
             frente a la mejora selectiva en TBR-2, especialmente marcada
             en DiaTrend.}
    \label{fig:profile_undersampling}
\end{figure}
...
y de magnitud similar en TBR-1 y TBR-2. La excepción parcial es TBR-2,
donde las técnicas de \textit{undersampling} (especialmente
PA-Undersampling, $39{,}83$\,mg/dL) tienden a mejorar ligeramente el RMSE
respecto a las de \textit{oversampling} (SMOTE, $38{,}74$\,mg/dL en este
rango), aunque la diferencia es pequeña y el patrón no es consistente entre
técnicas dentro de la misma familia.

En \textbf{DiaTrend} el perfil por rango es el más informativo. Las técnicas
de \textit{undersampling} (figura~\ref{fig:profile_undersampling}) presentan
el RMSE más alto en TIR y TAR ---consecuencia de la drástica reducción del
volumen de entrenamiento--- pero el más bajo en TBR-2
(PA-Undersampling: $71{,}34$\,mg/dL frente a $75{,}28$\,mg/dL del
\textit{baseline}). Las técnicas de \textit{oversampling}
(figura~\ref{fig:profile_oversampling}) invierten este patrón: mejoran TIR
y TAR pero degradan TBR-2 (ROS: $77{,}31$\,mg/dL). Las técnicas
\textit{same\_size} (figura~\ref{fig:profile_samesize}) presentan un perfil
intermedio en todos los rangos, sin la mejora en TBR-2 del
\textit{undersampling} ni la mejora en TIR del \textit{oversampling}.
Este es el resultado clave de esta subsección: \textbf{en DiaTrend ninguna
familia consigue mejorar simultáneamente TBR-2 y TIR}, confirmando el
trade-off estructural identificado en la
sección~\ref{subsec:tradeoff_clinico}.

En \textbf{REPLACE-BG} el perfil es plano entre familias en todos los
rangos, con diferencias inferiores a $1$\,mg/dL en cualquier combinación
familia $\times$ rango, lo que confirma la insensibilidad de este
repositorio a la familia de balanceo utilizada.
...
\paragraph{CD diagrams: significación estadística.}
Por estricto rigor metodológico, los diagramas de diferencia crítica
(CD) para \textbf{T1DiabetesGranada} y \textbf{REPLACE-BG} se omiten
de esta sección al carecer de validez inferencial: el test de Friedman
previo no rechaza la hipótesis nula en ninguno de los dos repositorios
($p = 0{,}437$ y $p = 0{,}779$, respectivamente), por lo que la barra
de diferencia crítica ($\mathrm{CD} = 13{,}25$) engloba la totalidad
de las 21 condiciones evaluadas sin que ningún par de técnicas resulte
estadísticamente distinguible. Incluir dichos diagramas induciría al
lector a extraer conclusiones espurias sobre la superioridad de alguna
técnica; la equivalencia estadística observada confirma que las
fluctuaciones de error en ambos repositorios son atribuibles a la
varianza estocástica del proceso de entrenamiento y validación cruzada,
y no a un efecto sistemático del reequilibrio demográfico.

En \textbf{DiaTrend}, en cambio, el test de Friedman rechaza la
hipótesis nula con $p < 0{,}0001$ ($\chi^2_F = 85{,}71$), habilitando
la interpretación del \textit{post-hoc} de Nemenyi. El CD diagram
(Figura~\ref{fig:cd_rmse_diatrend}) muestra una división clara entre
dos grupos: las técnicas de dimensión edad con fuerte submuestreo
(Age\textperiodcentered{}PAUndersampling, posición $\approx$20;
Age\textperiodcentered{}Undersampling, posición $\approx$19;
Age\textperiodcentered{}Jittering, posición $\approx$18) se sitúan
en el extremo derecho del diagrama (peor RMSE), estadísticamente
distintas del grupo de técnicas de dimensión sexo y del
\textit{baseline} original. Las técnicas de dimensión sexo forman
un grupo compacto en las posiciones intermedias ($\approx$3--10),
estadísticamente indistinguibles entre sí y del \textit{baseline}.
Age\textperiodcentered{}Tomek Links ocupa la posición~1 del ranking
y se separa significativamente de Age\textperiodcentered{}PAUndersampling,
Age\textperiodcentered{}Undersampling y Age\textperiodcentered{}Jittering,
pero no del resto de técnicas.
...
\begin{figure}[htbp]
    ...
    \caption{Diagrama de diferencia crítica (Nemenyi) para el RMSE
             global en \textbf{DiaTrend} ($p < 0{,}0001$,
             $\chi^2_F = 85{,}71$). Los diagramas análogos para
             T1DiabetesGranada y REPLACE-BG se omiten por no alcanzar
             significación en el test de Friedman previo ($p = 0{,}437$
             y $p = 0{,}779$, respectivamente), lo que impide cualquier
             interpretación inferencial de los rangos medios.}
    \label{fig:cd_rmse_diatrend}
\end{figure}

\paragraph{CD diagram TBR-2: hipoglucemia severa en DiaTrend.}
En el rango TBR-2, el test de Friedman rechaza igualmente la hipótesis
nula en DiaTrend ($p = 0{,}00013$, $\chi^2_F = 51{,}56$); los
repositorios restantes vuelven a quedar fuera del análisis inferencial
por los mismos motivos expuestos anteriormente. La lectura del CD diagram
(Figura~\ref{fig:cd_rmse_tbr2_diatrend}) revela un reordenamiento
sustancial respecto al de ENTIRE: las técnicas de aumento
(Age\textperiodcentered{}ROS+Tomek, Age\textperiodcentered{}SMOTE,
Age\textperiodcentered{}Oversampling) escalan a las primeras posiciones
(1--3), mientras que Age\textperiodcentered{}PAUndersampling y
Age\textperiodcentered{}Undersampling caen a las últimas (20--21),
siendo estadísticamente distintos del grupo de técnicas de aumento.
Este reordenamiento confirma el \textit{trade-off} estructural
identificado en la Sección~\ref{subsec:tradeoff_clinico}: las
técnicas que mejoran el RMSE global en DiaTrend (submuestreo) lo
empeoran en TBR-2, y viceversa. Sex\textperiodcentered{}PAUndersampling
ocupa la posición~1 del ranking en TBR-2, sin diferenciarse
significativamente de las técnicas de aumento de dimensión edad pero
sí de Age\textperiodcentered{}PAUndersampling y
Age\textperiodcentered{}Undersampling, lo que sugiere que el balanceo
por sexo con submuestreo produce un efecto más moderado en TBR-2 que
el balanceo por edad.
...
\begin{figure}[htbp]
    ...
    \caption{Diagrama de diferencia crítica (Nemenyi) para el RMSE
             en el rango TBR-2 (hipoglucemia severa, $<$54\,mg/dL)
             en \textbf{DiaTrend} ($p = 0{,}00013$,
             $\chi^2_F = 51{,}56$). Se omiten los diagramas de
             T1DiabetesGranada ($p = 0{,}120$) y REPLACE-BG
             ($p = 0{,}140$) por no significación del test de
             Friedman previo.}
    \label{fig:cd_rmse_tbr2_diatrend}
\end{figure}

\paragraph{Ranking global mediante el método Borda.}
La figura~\ref{fig:borda_lollipop} agrega los rankings individuales en
una puntuación global que resume el posicionamiento de cada técnica a
través de datasets, métricas y rangos glucémicos.
...
\paragraph{Estabilidad del ranking entre métricas.}
La figura~\ref{fig:bump_chart_entire} examina si el ordenamiento de las
técnicas es robusto al cambio de métrica de evaluación, comparando el
ranking por RMSE y por MAE para el conjunto completo.
...
\begin{figure}[htbp]
    ...
    \caption{\textit{Bump chart} mostrando la estabilidad del ranking
             ...
             inclinadas indican comportamiento dispar según la métrica.}
...
Esta sección sintetiza los hallazgos de las secciones anteriores y responde de
forma directa a las preguntas de investigación que motivaron el trabajo. Cada
respuesta integra la evidencia del análisis del \textit{train set}
(Sección~\ref{sec:efecto_balanceo_trainset}) con la del análisis de predicción
(Sección~\ref{sec:analisis_mejora}), y distingue entre mejora estadística y
relevancia clínica cuando ambas dimensiones no coinciden.

% ---------------------------------------------------------
\paragraph{¿Mejora el balanceo demográfico el rendimiento del modelo LSTM?}
% ---------------------------------------------------------

La respuesta es matizada y fuertemente dependiente del repositorio y del rango
glucémico que se priorice. En términos globales (RMSE sobre ENTIRE), el balanceo
demográfico no produce mejoras estadísticamente significativas en
T1DiabetesGranada ($p = 0{,}120$, Friedman) ni en REPLACE-BG ($p = 0{,}140$):
en ambos repositorios las diferencias observadas entre técnicas son estadísticamente
compatibles con variación aleatoria, y la evidencia de que cualquier técnica supere
al \textit{baseline} original es insuficiente. En DiaTrend, en cambio, el test de
Friedman rechaza la hipótesis nula en todos los rangos y métricas evaluados
($p < 0{,}0001$ en ENTIRE RMSE), y el post-hoc de Nemenyi identifica grupos de
técnicas estadísticamente distintos: las técnicas de submuestreo agresivo por edad
(RUS, PA-Undersampling) deterioran significativamente el rendimiento global respecto
a Tomek Links y a las técnicas de dimensión sexo, mientras que estas últimas son
estadísticamente equivalentes al \textit{baseline}.

La lectura cambia al focalizar la evaluación en TBR-2 (hipoglucemia severa,
$<54$\,mg/dL), el rango clínicamente más crítico. En DiaTrend, donde el test de
Friedman también es significativo en TBR-2 ($p = 0{,}00013$), el ranking se
invierte: las técnicas de aumento (ROS, SMOTE, ROS+Tomek) mejoran el RMSE en
TBR-2 respecto al \textit{baseline}, mientras que las de submuestreo lo empeoran.
Esta inversión sistemática confirma un \textbf{trade-off estructural}: en DiaTrend
no existe ninguna técnica que mejore simultáneamente el RMSE global y el RMSE en
hipoglucemia severa, lo que obliga a elegir entre optimizar para el rendimiento
medio o para el rango clínicamente más prioritario.

% ---------------------------------------------------------
\paragraph{¿Por qué T1DiabetesGranada es insensible al balanceo?}
\label{subsec:discusion_granada}
% ---------------------------------------------------------

La ausencia de efecto estadísticamente significativo en T1DiabetesGranada no
refleja un fallo del \textit{pipeline} de balanceo, sino un fenómeno de interés
propio para el aprendizaje profundo biomédico: la existencia de un
\textbf{límite de saturación representacional}. Con más de 13 millones de ventanas
de entrenamiento generadas por 643 pacientes, el volumen de datos es suficientemente
masivo para que la red LSTM adquiera, de forma implícita, cierta invarianza frente a
las fluctuaciones demográficas. En este régimen de abundancia, el desequilibrio
moderado de 3{,}4:1 entre sexos queda neutralizado por la riqueza y diversidad
intrínseca de las trayectorias de glucosa: la red aprende representaciones
suficientemente generales sin necesidad de que el conjunto de entrenamiento sea
reequilibrado artificialmente.

Este resultado apunta a la existencia de un \textbf{umbral crítico de volumen} a
partir del cual el balanceo demográfico explícito deviene superfluo. La implicación
práctica es relevante en el contexto de los sistemas de páncreas artificial: cuando
se dispone de cohortes de gran escala del mundo real, la carga computacional
asociada al preprocesamiento de balanceo puede omitirse sin comprometer la equidad
del modelo, lo que simplifica el \textit{pipeline} de despliegue clínico en
entornos de lazo cerrado.

% ---------------------------------------------------------
\paragraph{¿Por qué REPLACE-BG es insensible al balanceo?}
% ---------------------------------------------------------

El mecanismo subyacente en REPLACE-BG es distinto, y obedece al diseño propio de
un ensayo clínico aleatorizado y controlado. Al imponer una duración de seguimiento
idéntica para todos los participantes ($\approx 182$ días) y protocolos de
adherencia estrictos, el estudio elimina la principal fuente de desequilibrio que
aparece de forma natural en datos de vida libre: la amplificación del desbalanceo
demográfico al pasar de paciente a ventana (véase Subsección~\ref{subsec:imbalance_paciente_vs_ventana}).
Cuando todos los pacientes contribuyen con un número similar de ventanas al
conjunto de entrenamiento, el modelo LSTM queda estructuralmente protegido frente
a los sesgos de representación, haciendo que cualquier intervención correctiva
a posteriori resulte estadísticamente indetectable.

Este fenómeno se explica por la \textbf{estabilidad proporcional inducida por el diseño experimental}: el protocolo estandarizado de seguimiento actúa como un regulador implícito de la representación de las ventanas, de modo que el balanceo explícito no añade información que la distribución de los datos de origen no contenga ya.
El hallazgo tiene una lectura normativa clara: una adecuada planificación en la
fase de adquisición y curación de datos es la estrategia más eficiente para mitigar
sesgos algorítmicos, y resulta preferible a cualquier intervención correctiva sobre
datos ya recolectados.

% ---------------------------------------------------------
\paragraph{Tres regímenes de eficacia del balanceo}
% ---------------------------------------------------------

Los tres repositorios ilustran conjuntamente tres regímenes cualitativamente
distintos en los que el balanceo demográfico opera de forma diferente:

\begin{enumerate}
    \item \textbf{Régimen de saturación} (T1DiabetesGranada): el volumen de datos
    supera el umbral a partir del cual el modelo adquiere robustez demográfica de
    forma implícita. El balanceo es inocuo pero innecesario.

    \item \textbf{Régimen de equilibrio por diseño experimental (REPLACE-BG):} la estandarización del protocolo del estudio garantiza una distribución de ventanas homogénea. El balanceo no encuentra un desequilibrio estructural significativo que requerir corrección.

    \item \textbf{Régimen de desequilibrio activo} (DiaTrend): el dataset de vida
    libre presenta heterogeneidad en la duración de las series y desequilibrios
    demográficos que se amplifican al nivel de ventana. Aquí el balanceo sí tiene
    efecto medible, aunque con un trade-off clínico inevitable entre rendimiento
    global y rendimiento en hipoglucemia severa.
\end{enumerate}

Esta taxonomía constituye, en sí misma, una contribución metodológica del trabajo:
proporciona un marco interpretativo para anticipar cuándo el balanceo demográfico
puede esperarse que sea efectivo en problemas de predicción sobre series temporales
clínicas, y cuándo resulta redundante.
