...
ME HE QUEDAOD POR AQUI,LA RESPUESTA DE ABAJO ESTA EN CLAUDE , HAY QUE TRAERLA
...
% ---------------------------------------------------------
% FIGURAS (family_size):
%   - distributions/family_size/oversampling/dist_RMSE_ENTIRE_oversampling.pdf
%   - distributions/family_size/undersampling/dist_RMSE_ENTIRE_undersampling.pdf
%   - distributions/family_size/same_size/dist_RMSE_ENTIRE_same_size.pdf
%   - profiles/family_size/oversampling/range_profile_facet_RMSE_oversampling.pdf
%   - profiles/family_size/undersampling/range_profile_facet_RMSE_undersampling.pdf
%   - profiles/family_size/same_size/range_profile_facet_RMSE_same_size.pdf
%
% JUSTIFICACIÓN: La familia same_size actúa como grupo de control que aisla
%   el efecto del reequilibrio del efecto del volumen. Comparar oversampling
%   vs. same_size vs. undersampling permite responder: «¿la mejora/deterioro
%   observada se debe al balanceo o al cambio de tamaño del train?»

% TODO: Párrafo de encuadre:
%   «Como se argumentó en la sección~\ref{subsec:impacto_tamaño}, el cambio
%    de tamaño del conjunto de entrenamiento es un confundidor potencial de los
%    resultados de predicción. Para desacoplarlo, las técnicas se agrupan en
%    tres familias según su efecto sobre el tamaño del train: \textit{oversampling}
%    (incrementan el tamaño), \textit{undersampling} (lo reducen) y \textit{same\_size}
%    (lo mantienen aproximadamente constante). La comparación sistemática entre
%    familias permite atribuir las diferencias de rendimiento al mecanismo de
%    balanceo con mayor rigor.»

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/distributions/family_size/oversampling/dist_RMSE_ENTIRE_oversampling}
        \caption{Oversampling}
        \label{fig:dist_rmse_oversampling}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/distributions/family_size/same_size/dist_RMSE_ENTIRE_same_size}
        \caption{Same size}
        \label{fig:dist_rmse_samesize}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/distributions/family_size/undersampling/dist_RMSE_ENTIRE_undersampling}
        \caption{Undersampling}
        \label{fig:dist_rmse_undersampling}
    \end{subfigure}
    \caption{Distribución del RMSE (\textit{ENTIRE}) por familia de tamaño: técnicas que incrementan el conjunto de entrenamiento (\textit{oversampling}), que lo mantienen constante (\textit{same size}) y que lo reducen (\textit{undersampling}). La comparación entre los tres paneles permite separar el efecto del volumen de datos del efecto del reequilibrio demográfico.}
    \label{fig:dist_rmse_family_size}
\end{figure}

% TODO: Comentar la figura~\ref{fig:dist_rmse_family_size}.
...
%    esperar que las técnicas de oversampling superasen sistemáticamente a las de
%    same_size y estas a las de undersampling. La figura~\ref{fig:dist_rmse_family_size}
%    [confirma/contradice] esta hipótesis: [describir el patrón observado].»
%   PÁRRAFO 2:
%   «En particular, para [dataset], las técnicas same_size obtienen un RMSE
%    [similar a/mejor que/peor que] las de oversampling, lo que sugiere que
%    [el reequilibrio por sí solo aporta valor / el efecto es principalmente
%    volumétrico / ningún mecanismo produce mejora neta]. Este resultado tiene
%    implicaciones directas para la elección de técnicas en la práctica: [conclusión].»

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/profiles/family_size/oversampling/range_profile_facet_RMSE_oversampling}
        \caption{Oversampling}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/profiles/family_size/same_size/range_profile_facet_RMSE_same_size}
        \caption{Same size}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/profiles/family_size/undersampling/range_profile_facet_RMSE_undersampling}
        \caption{Undersampling}
    \end{subfigure}
    \caption{Perfil de RMSE por rango glucémico y familia de tamaño. Cada panel muestra el RMSE medio sobre los \textit{folds} para cada rango glucémico (TBR\_2, TBR\_1, TIR, TAR\_1, TAR\_2) y \textit{dataset}. El perfil revela si el efecto del balanceo es uniforme entre rangos o si hay rangos que se benefician (o perjudican) selectivamente.}
    \label{fig:profile_family_size}
\end{figure}

% TODO: Comentar la figura~\ref{fig:profile_family_size}.
%   «¿El undersampling empeora el RMSE uniformemente en todos los rangos,
%    o lo empeora selectivamente en TIR (rango con más ventanas y más fácil
%    de predecir) mientras lo mejora en TBR_2? ¿El oversampling mejora TBR_2
%    de forma consistente entre datasets? ¿El same_size presenta un perfil
%    intermedio o se acerca más a uno de los otros dos?»
...
% ---------------------------------------------------------
% FIGURAS:
%   - rankings/cd_diagram_RMSE_ENTIRE_T1DiabetesGranada.pdf
%   - rankings/cd_diagram_RMSE_ENTIRE_REPLACE-BG.pdf
%   - rankings/cd_diagram_RMSE_ENTIRE_DIATREND.pdf
%   - rankings/cd_diagram_RMSE_TBR_2_T1DiabetesGranada.pdf
%   - rankings/cd_diagram_RMSE_TBR_2_REPLACE-BG.pdf
%   - rankings/cd_diagram_RMSE_TBR_2_DIATREND.pdf
%   - rankings/borda_lollipop.pdf
%   - rankings/bump_chart_ENTIRE.pdf
%
% ORDEN: primero los CD diagrams de ENTIRE (significación global),
%   luego los de TBR_2 (significación clínica), y finalmente el
%   Borda lollipop y el bump chart como síntesis de ranking global.

% TODO: Párrafo introductorio sobre la prueba de Friedman:
%   «Para determinar si las diferencias observadas entre técnicas son
%    estadísticamente significativas, se aplica la prueba no paramétrica
%    de Friedman (ver sección~\ref{sec:protocolo_evaluacion}), con los 5
%    \textit{folds} como bloques y las técnicas como tratamientos. Cuando
%    la prueba de Friedman rechaza la hipótesis nula, se aplica la corrección
%    post-hoc de Nemenyi para identificar qué pares de técnicas difieren
%    significativamente. Los resultados se representan mediante diagramas de
%    diferencia crítica (CD diagrams), en los que técnicas unidas por una
%    barra horizontal no difieren significativamente al nivel $\alpha = 0{,}05$.»

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/cd_diagram_RMSE_ENTIRE_T1DiabetesGranada}
        \caption{T1DiabetesGranada}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/cd_diagram_RMSE_ENTIRE_REPLACE-BG}
        \caption{REPLACE-BG}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/cd_diagram_RMSE_ENTIRE_DIATREND}
        \caption{DiaTrend}
    \end{subfigure}
    \caption{Diagramas de diferencia crítica (Nemenyi post-hoc) para el RMSE sobre el conjunto completo (\textit{ENTIRE}). Las técnicas se ordenan por rango medio (menor a la izquierda = mejor rendimiento). Técnicas conectadas por una barra horizontal no difieren significativamente ($\alpha = 0{,}05$). El test de Friedman previo se rechaza si el $p$-valor es inferior al nivel de significación.}
    \label{fig:cd_rmse_entire}
\end{figure}

% TODO: Comentar la figura~\ref{fig:cd_rmse_entire} para los tres datasets.
%   «En T1DiabetesGranada, la prueba de Friedman [rechaza/no rechaza] la
%    hipótesis nula ($p = $ [valor]). [Si rechaza:] El diagrama de Nemenyi
%    muestra que [técnica A] difiere significativamente de [técnica B], mientras
%    que el grueso de las técnicas forma un grupo estadísticamente indistinguible.
%    [Si no rechaza:] No existe evidencia estadística de que ninguna técnica
%    mejore significativamente al original en este dataset.»
%   Repetir para REPLACE-BG y DiaTrend.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/cd_diagram_RMSE_TBR_2_T1DiabetesGranada}
        \caption{T1DiabetesGranada}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/cd_diagram_RMSE_TBR_2_REPLACE-BG}
        \caption{REPLACE-BG}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/cd_diagram_RMSE_TBR_2_DIATREND}
        \caption{DiaTrend}
    \end{subfigure}
    \caption{Diagramas de diferencia crítica (Nemenyi) para el RMSE evaluado exclusivamente en el rango TBR\_2 (hipoglucemia severa). Compárese con la figura~\ref{fig:cd_rmse_entire}: ¿el orden de las técnicas cambia cuando se focaliza la evaluación en el rango clínicamente más crítico?}
    \label{fig:cd_rmse_tbr2}
\end{figure}

% TODO: Comentar la figura~\ref{fig:cd_rmse_tbr2}.
%   Esta es la figura más relevante desde el punto de vista clínico.
%   «¿Hay técnicas que escalen posiciones al evaluar en TBR_2 respecto a ENTIRE,
%    indicando que son especialmente efectivas para la hipoglucemia severa?
%    ¿Hay técnicas que bajan de posición, revelando que su buen rendimiento
%    global se diluye en el rango crítico?»

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../../analysis_relief/outputs/figures/rankings/borda_lollipop}
    \caption{Ranking global de técnicas de balanceo mediante el método Borda (suma de rangos a través de \textit{datasets}, métricas y rangos glucémicos). Una técnica con Borda más alto ocupa posiciones superiores del ranking de forma más consistente entre todos los escenarios evaluados. El gráfico \textit{lollipop} facilita la comparación de la puntuación acumulada.}
    \label{fig:borda_lollipop}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../../analysis_relief/outputs/figures/rankings/bump_chart_ENTIRE}
    \caption{\textit{Bump chart} mostrando la evolución del ranking de cada técnica entre los tres \textit{datasets} para el RMSE en el conjunto completo (\textit{ENTIRE}). Técnicas con líneas horizontales mantienen su posición relativa entre datasets (comportamiento estable); técnicas con líneas muy inclinadas cambian de posición (comportamiento dataset-dependiente).}
    \label{fig:bump_chart_entire}
\end{figure}

% TODO: Comentar conjuntamente las figuras~\ref{fig:borda_lollipop}
%   y~\ref{fig:bump_chart_entire}.
%   «El ranking Borda identifica [técnica X] como la más consistentemente
%    bien posicionada entre los tres datasets y todas las métricas evaluadas.
%    Sin embargo, el bump chart revela que [ninguna técnica / la técnica Y]
%    mantiene el primer puesto en todos los datasets: en [dataset A] predomina
%    [técnica P] mientras que en [dataset B] lo hace [técnica Q], lo que
%    sugiere que la técnica óptima no es universal sino dataset-dependiente.
%    Esta conclusión es consistente con la heterogeneidad estructural de los
%    datasets documentada en el capítulo~\ref{chapter:eda}.»
...
% TODO: Esta sección sintetiza los hallazgos de las dos anteriores y responde
%   a las preguntas de investigación del trabajo. Estructura sugerida:

\paragraph{¿Mejora el balanceo demográfico el rendimiento del modelo LSTM?}
% TODO: Respuesta directa a la pregunta de investigación principal, apoyada
%   en los resultados estadísticos (Friedman + Nemenyi) y en los heatmaps
%   de delta. Distinguir entre mejora global (ENTIRE) y mejora clínica (TBR_2).
%   «La respuesta es [matizada]: el balanceo demográfico no produce mejoras
%    estadísticamente significativas en el rendimiento global en [datasets],
%    pero sí produce mejoras selectivas en el rango TBR_2 para [técnicas]
%    en [datasets], con significación estadística ($p < 0{,}05$, Nemenyi).»

\paragraph{¿Qué familia de técnicas ofrece mejor relación beneficio/coste?}
% TODO: Comparar oversampling vs. same_size vs. undersampling en términos de
%   (1) rendimiento en TBR_2, (2) estabilidad inter-fold y (3) coste computacional
%   implícito (el undersampling reduce el train, el oversampling lo aumenta).
%   Vincular con la figura~\ref{fig:dist_rmse_family_size}.

\paragraph{¿Es el efecto del balanceo dataset-dependiente?}
% TODO: Discutir por qué los resultados difieren entre T1DiabetesGranada,
%   REPLACE-BG y DiaTrend. Vincular con las características estructurales
%   de cada dataset documentadas en el capítulo de EDA:
%   - T1DGranada: dataset grande, desequilibrio moderado → efecto del balanceo limitado.
%   - REPLACE-BG: seguimientos homogéneos → predicibilidad alta pero poco margen de mejora.
%   - DiaTrend: dataset pequeño, desequilibrio extremo → alta variabilidad, resultados inestables.

\paragraph{¿Qué dimensión demográfica —edad o sexo— es más relevante?}
% TODO: Vincular con la figura~\ref{fig:slope_rmse_dim} y con los heatmaps
%   demográficos del train set. ¿Balancear por edad produce resultados
%   sistemáticamente distintos a balancear por sexo, o el efecto es equivalente?
%   Discutir en términos del ratio de desequilibrio de cada dimensión (sección~\ref{sec:deteccion_imbalance}).

\paragraph{Limitaciones del análisis.}
% TODO: Mencionar las principales limitaciones:
%   1. El número de folds (5) limita la potencia del test de Friedman para
%      detectar diferencias pequeñas.
%   2. DiaTrend tiene un tamaño muestral insuficiente para extraer conclusiones
%      generalizables, como indica la alta variabilidad inter-fold documentada.
%   3. El análisis se restringe a un único modelo (LSTM) y un único horizonte
%      de predicción (PH=4); los resultados pueden diferir para otros modelos
%      u horizontes.
%   4. El efecto del balanceo sobre el conjunto de entrenamiento (sección~\ref{sec:efecto_balanceo_trainset})
%      se ha cuantificado a nivel de distribución glucémica, pero no se ha analizado
%      el efecto sobre la distribución de las series temporales (autocorrelación,
%      estacionariedad), que podría modular los resultados del LSTM.