...
def generate_main_results_table(master: pd.DataFrame, metric: str = "RMSE") -> None:
    """
    Tabla principal: filas = condición, columnas = dataset × rango.
    Valores: mean ± std. Mejor valor en negrita.
    Genera .csv (fácil de importar) y .tex (listo para insertar en LaTeX).
    """
    ...
    csv_path = TABLES_DIR / f"main_results_{metric}.csv"
    df_csv.to_csv(csv_path, index=False)
    log.info(f"  CSV guardado: {csv_path.name}")

    # ---- LaTeX ----
    n_cols   = len(datasets) * len(ranges)
    col_spec = "l" + "r" * n_cols
    lines    = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\small",
        r"\caption{" + f"Results by condition — {metric} (mean \\pm std across 5 folds)" + r"}",
        r"\label{tab:results_" + metric.lower() + r"}",
    ...
    for ds in datasets:
        ds_headers.append(
            r"\multicolumn{" + str(len(ranges)) + r"}{c}{"
            + r"\textbf{" + DATASET_LABELS.get(ds, ds) + r"}" + r"}"
        )
    ...
    col_start = 2
    for ds in datasets:
        cmidrules.append(f"\\cmidrule(lr){{{col_start}-{col_start+len(ranges)-1}}}")
        col_start += len(ranges)
    ...
    # Find best (non-original) value per (dataset, range)
    best_vals = {}
    ...
    for cond in conds:
        ...
        cells = [r"\textit{" + _cond_label(cond) + r"}"
                 if cond == "original" else _cond_label(cond)]
    ...
    tex_path = TABLES_DIR / f"main_results_{metric}.tex"
    tex_path.write_text("\n".join(lines), encoding="utf-8")
    log.info(f"  LaTeX guardado: {tex_path.name}")
...
def generate_ranking_table(master: pd.DataFrame, metric: str = "RMSE",
                           rng: str = "ENTIRE") -> None:
    """
    Tabla de ranking: filas = condición, columnas = dataset.
    Valor = posición en el ranking (1=mejor) con la media entre paréntesis.
    """
    sub = master[
    ...
    out_csv = TABLES_DIR / f"ranking_{metric}_{rng}.csv"
    df.to_csv(out_csv, index=False)
    log.info(f"  Ranking table CSV: {out_csv.name}")
...
def generate_delta_table(master: pd.DataFrame, metric: str = "RMSE",
                         rng: str = "ENTIRE") -> None:
    """
    Tabla de deltas porcentuales (improvement_pct).
    Verde = mejora, rojo = degradación (en LaTeX via xcolor).
    """
    sub = master[
        (~master["is_original"]) &
        (master["metric"] == metric) &
    ...
    lines = [
        ...
        r"\label{tab:delta_" + f"{metric}_{rng}".lower() + r"}",
        r"\begin{tabular}{l" + "r" * len(datasets) + r"}",
        r"\toprule",
        r"\textbf{Condition} & " +
        " & ".join(r"\textbf{" + DATASET_LABELS.get(d,d) + r"}" for d in datasets) + r" \\",
        r"\midrule",
    ]
    ...
    for cond in conds:
        ...
        for ds in datasets:
            ...
            if np.isnan(v):
                ...
                sign = "+" if v > 0 else ""
                # Use textcolor in LaTeX if xcolor is loaded
    ...
    tex_path = TABLES_DIR / f"delta_{metric}_{rng}.tex"
    tex_path.write_text("\n".join(lines), encoding="utf-8")
    log.info(f"  Delta table LaTeX: {tex_path.name}")


# ── T4: Clarke zones table ────────────────────────────────────────────────────

def generate_clarke_table(master: pd.DataFrame) -> None:
    """
    Tabla con % puntos en zona A y A+B por condición × dataset.
    Mayor = mejor para ambas métricas.
    """
    sub = master[
        (master["metric"].isin(["A", "A + B"])) &
        (master["range"] == "ENTIRE")
    ].copy()
    if sub.empty:
        log.warning("Clarke table: sin datos de zonas CEG")
        return
        ...
        for ds in datasets:
            for zone in ["A", "A + B"]:
                ...
                if cell.empty:
                    ...
                    s = cell["std"].values[0]
                    row[f"{DATASET_LABELS.get(ds,ds)} Zone {zone}"] = \
                        f"{m:.1f} ± {s:.1f}" if not np.isnan(m) else "—"
        rows.append(row)

    df = pd.DataFrame(rows)
    out = TABLES_DIR / "clarke_zones.csv"
    df.to_csv(out, index=False)
    log.info(f"  Clarke zones table: {out.name}")
